...
import { getStudioPublicationStatus } from "@/lib/studio-publication-control";

export async function GET(request: Request, { params }: { params: { publicationId: string } }) {
  try {
    assertStudioApiKey(request);
    const status = await getStudioPublicationStatus(params.publicationId);
    if (!status) {
